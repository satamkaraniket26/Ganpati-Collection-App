...
async function loadPendingRecovery() {
    ...
    try {
        ...
        const response = await fetch(API_URL + "?action=pendingRecovery");
        ...
        const container = document.getElementById("pendingRecoveryList");
        ...
        if (data.length === 0) {
            ...
            container.innerHTML =
                "<div class='search-card'><h3>No Pending Recoveries</h3></div>";
...
function openRecovery(index){
    ...
    if(item.sheet==="Vendor"){
        ...
        document.getElementById("building").value=item.shopName;
        document.getElementById("wing").value="-";
        document.getElementById("roomNo").value="-";
...
async function saveRecovery(){
    ...
    document.getElementById("recoveredAmount").value.trim();
...
async function loadDashboard() {
    ...
    try {
        ...
        document.getElementById("recoveryTotal").innerHTML =
            "₹" + Number(data.recovery || 0).toLocaleString();
        ...
        document.getElementById("cashCollection").innerHTML =
            "₹" + Number(data.overallCashCollection || 0).toLocaleString();

        document.getElementById("onlineCollection").innerHTML =
            "₹" + Number(data.overallOnlineCollection || 0).toLocaleString();
        ...
        document.getElementById("totalEntries").innerHTML =
            Number(data.totalEntries || 0).toLocaleString();
...
async function loadHomeDashboard() {
    ...
    try {
        ...
        const overallCollection =
            Number(dashboard.paidVargani || 0) +
            Number(dashboard.politicalPaid || 0) +
            Number(dashboard.vendorPaid || 0) +
        ...
        document.getElementById("pendingRecovery").innerHTML =
            "₹" + Number(dashboard.totalPending || 0).toLocaleString();
